...
int sortMoves(std::vector<Move>& moves, const Position& pos, int ply, Move::Color color) {
    std::sort(moves.begin(), moves.end(), [&](const Move& a, const Move& b) {
        ...
        // Prioritize TT Move
        if (a == ttBestMove) scoreA += 10000;
        if (b == ttBestMove) scoreB += 10000;

        // Killer Moves
        if (a == killerMoves[ply][0]) scoreA += 9000;
        if (b == killerMoves[ply][0]) scoreB += 9000;
        if (a == killerMoves[ply][1]) scoreA += 8000;
        if (b == killerMoves[ply][1]) scoreB += 8000;

        // History Heuristic
        // Ensure valid move indices before accessing the array
        int colorIndex = (color == Move::Color::WHITE) ? 0 : 1;
        ...
        if (a.isCapture) scoreA += 100 * getPieceValue(a.capturedPieceType) - getPieceValue(a.pieceType);
        if (b.isCapture) scoreB += 100 * getPieceValue(b.capturedPieceType) - getPieceValue(b.pieceType);
